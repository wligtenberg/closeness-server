...
from .models import User, Role, Temperature, RegistrationState, RegisteredDevice

# declare these globalish objects before initializing models
application = None


class closeness_server(Diamond):
...
def api_map(rest_extension):
    from .api import RegisteredDevice, Temperature
    rest_extension.add_resource(RegisteredDevice, '/register/<string:device_id>')
    rest_extension.add_resource(Temperature, '/temperature')


def create_app():
    global application
    if not application:
        application = closeness_server()
