...
SHELL=/bin/bash
PROJECT_NAME=closeness-server
MOD_NAME=closeness_server
...
server:
	SETTINGS=$$PWD/etc/conf/dev.conf bin/manage.py publicserver
...
db:
	...
	SETTINGS=$$PWD/etc/conf/dev.conf bin/manage.py user_add --email "admin@example.com" --password "viu" --admin
