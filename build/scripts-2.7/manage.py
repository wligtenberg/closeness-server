...
from closeness_server import create_app, db
from closeness_server.models import User, Role

app = create_app()
migrate = Migrate(app, db, directory="closeness_server/migrations")
...
def init_db(migration):
    ...
    if migration:
        ...
        db.create_all()
        db.session.commit()
        cfg = alembic.config.Config("closeness_server/migrations/alembic.ini")
